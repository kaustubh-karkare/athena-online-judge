...
Object.keys(schema).map(function(collection){
	rpc.on(collection+".create",function(socket,data,callback){
		var item, save;
		async.series([
	...
	rpc.on(collection+".modify",function(socket,data,callback){
	...
	rpc.on(collection+".delete",function(socket,data,callback){
	...
	rpc.on(collection+".select",function(socket,data,callback){
		var item, result;
		async.series([
			function(cb){ specification.match_partial(collection,schema[collection],data,function(e,i){ if(!e) item=i; cb(e); }); },
			function(cb){ database.select(collection,item,{},function(e,r){ if(!e) result=r; cb(e); }); }
		], function(error){ callback(error,result); });
	});
	rpc.on(collection+".suggest",function(socket,data,callback){
		...
		async.series([
			...
			function(cb){ database.page(collection,item,columns,{"page":0,"size":5,"sort":key},function(e,r){ if(!e) result=r; cb(e); }); }
		], function(error){ callback(error,result); });
	});
});
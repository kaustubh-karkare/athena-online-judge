...
exports = new widget(function(data,callback){
	...
	async.parallel([
		...
		$("<span class='pull-right'>").appendTo(heading).append(
			tabs.filter(function(x){
				return x!=="tutorial" || problem.tutorial!==null;
			}).map(function(x){
		...
		if(path[4]==="statement" || path[4]==="tutorial"){
			if(path[5]==="edit"){
				...
				var t = main.find("textarea")[0]; t.value = problem[path[4]];
				window.setTimeout(function(){
					...
					main.append($("<div style='text-align:center;margin-top:20px;'>").append([
						$("<a class='btn'>Update "+path[4].ucwords()+"</a>").click(function(){
							...
							rpc("problem.update",[problem._id,path[4],t.value],function(e,r){
				...
				main.append(plugin.comment({"location":"C"+contest._id+"P"+problem._id+path[4].substr(0,1).toUpperCase()}).node);
			}
		} else {
			var main = plugin.pagination({
				"rpc":"code.list",
				"page":{"size":25},
				"data":{"contest":contest._id,"problem":problem._id},
		...
		if(path[4]==="statement" && path[5]!=="edit"){
			...
			if(auth.user!==null){
				...
				submit.append([
					...
					rpc("code.submit",{"contest":{_id:contest._id},"problem":{_id:problem._id},"code":editor?editor.getValue():textarea[0].value,"language":lang.value()},function(e,r){
